...
class Group4Num extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
    ...
    if (filterNum && filterNum >= 1 <= 45) {
      // 2 Num Relation Chart
      const group2NumRelateHeader = [['Từ', 'Đến', 'Xhiện']];
      const group2NumRelateItems = _.flatten(
        _.map(numGroupShowFrequens, group => {
          return [
            [group.index[0], group.index[1], group.count],
            [group.index[1], group.index[2], group.count],
            [group.index[2], group.index[3], group.count],
          ];
        }),
        true
      );
    ...
    return (
      <div className="container">
        <h1 className="animated fadeIn">Phân tích nhóm 4 số</h1>
            ...
            {numGroupShowFrequens.length <= 0 && (
              <div className="list-group">
                <div className="list-group-item text-center animated fadeIn">
                  {`Không có bộ 4 số nào chứa ${parseInt(
                    filterNum
...
export default withTracker(() => {
  ...
  const numGroupFrequens = calcCombineFrequen(rounds, 4);
