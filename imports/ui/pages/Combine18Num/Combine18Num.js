...
// Styles
import './Combine18Num.scss';

class Combine18Num extends React.Component {
  constructor(props) {
    super(props);
  ...
  render() {
              ...
              <td key={num}>
                <span
                  ...
                  onClick={() => {
                    ...
                    if (newState && selectedNums.length < 18) {
      ...
      return (
        <div key="1" className="list-group">
          <div className="list-group-item text-center animated fadeIn">
            Chưa chọn đủ bộ 12 số
          </div>
        </div>
      );
    });

    if (selectedNums.length >= 18) {
      const combs18 = [
        'ABCDKQ',
        'ABEINP',
        'ABFHJM',
      ...
      const genRounds = Functions.generate(combs18, selectedNums);
    ...
    return (
      <div className="container analyse-all">
        <h1 className="animated fadeIn">Tổ hợp 18 số</h1>
        <h5 className="animated fadeIn mb-4">
          <i>* Có tất cả 18.564 bộ 6 số từ 18 số. Phát sinh 42 bộ số.</i>
        </h5>
        <div
...
Combine18Num.defaultProps = {
  rounds: null,
};

Combine18Num.propTypes = {
